...
const SECTIONS = {
  ...
  notifications: { label: 'Notificaciones',  icon: 'notifications', module: () => SectionNotifications },
};
...
function canAccess(sectionId) {
  return (SECTION_ROLES[sectionId] || ['admin']).includes(Auth.getRole());
}

let currentSection = null;

function navigate(id) {
  if (!SECTIONS[id] || !canAccess(id)) return;
  currentSection = id;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  navigate(SECTIONS[initial] && canAccess(initial) ? initial : firstAllowed);
  updatePendingCount();
});
