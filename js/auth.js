...
const Auth = (() => {
  ...
  // Si PocketBase exige 2FA devuelve { needsMfa: true, mfaId }
  ...
  // Paso 2a: solicitar OTP por correo
  ...
  // Paso 2b: verificar código OTP para completar 2FA
  async function completeMfaWithOtp(otpId, code, mfaId) {
    try {
      const res = await fetch(`${PB_URL}/api/collections/_superusers/auth-with-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ otpId, password: code, mfaId }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) return { success: false };
      _saveSession(data);
